...
| Documentation | LaTeX report (`report/main.tex`) |
...
- Group creation and membership tracking with a locking mechanism for critical operations.
...
1. **Create group** – `createGroup()` creates a new group with a list of initial members.
2. **Join group** – `joinGroup()` allows a new member to join an existing group.
